...
1. **`wrangler.toml`** (指定 CF 编译目录)：
   name = "navsphere"
   compatibility_date = "2024-11-11"
   compatibility_flags = ["nodejs_compat"]
   pages_build_output_dir = ".vercel/output/static"

2. **`src/app/page.tsx`** (修复首页动态数据读取 Bug)：
   - 将原作者硬编码的静态 JSON 引用修改为从 `getFileContent()` 动态读取。

3. **`src/components/site-favicon.tsx`** (图片 CDN 自动转换)：
   - 增加了自动转换逻辑：将自动获取/上传的 `/assets/xxx.png` 相对路径自动转换为 `https://cdn.jsdelivr.net/...` CDN 链接，无需手动粘贴 URL。

4. **`src/components/navigation-content.tsx`** (顶部图标精简)：
...
访问 [GitHub Developer Settings → OAuth Apps](https://github.com/settings/developers)：
...
- 检查 GitHub OAuth App 的 Callback URL 是否与当前域名**完全匹配**。
...
### Q4：更换域名时需要修改什么？
1. **GitHub OAuth App**：更新 Homepage URL 和 Callback URL。
2. **CF Pages 环境变量**：更新 `NEXTAUTH_URL` 和 `NEXT_PUBLIC_API_URL`。
3. **重新部署** CF Pages 项目。
